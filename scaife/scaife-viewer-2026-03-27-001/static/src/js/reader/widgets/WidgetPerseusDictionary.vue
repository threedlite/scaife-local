    ...
    <div slot="body">
      <div>
        ...
        <details
          ...
          :key="result.urn || result.id || i"
          :open="i === 0"
          class="mt-2"
        >
          <summary><strong>{{ result.headword }}</strong></summary>
          <!-- Rich nested senses (upstream atlas shape) -->
          <div v-if="result.data && result.data.senses">
          ...
          <!-- Flat intro_text (local dictionaries app shape) -->
          <div v-else class="dict-body">{{ result.intro_text }}</div>
        </details>
      </div>
...
export default {
  ...
  watch: {
    ...
    query: "updateSearch",
    // When the user changes the dropdown, re-run the current query against
    // the newly selected dictionary. Skip if no query yet — otherwise we'd
    // dump the first page of an unrelated dictionary onto them.
    dictionary() {
      if (this.q) {
        this.updateSearch();
      }
