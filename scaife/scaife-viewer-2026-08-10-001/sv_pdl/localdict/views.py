...
def dictionary_entries(request, slug):
    ...
    results = [
        {
            "id": e.id,
            # WidgetPerseusDictionary keys on `urn` and dereferences
            # `result.data.senses`. We have no sense hierarchy, so `data` is
            # an empty dict: the widget's `(result.data.senses || [])` then
            # yields [] and it renders the flat `intro_text` branch instead.
            # Emitting these keeps the widget unpatched against upstream.
            "urn": f"{d.urn}:{e.id}",
            "data": {},
            "headword": e.headword,
