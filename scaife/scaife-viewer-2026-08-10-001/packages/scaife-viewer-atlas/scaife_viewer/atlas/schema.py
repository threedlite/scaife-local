...
class LimitedConnectionField(DjangoFilterConnectionField):
    ...
    # LOCAL CHANGE (2026-08-14): signature updated for graphene-django 2.16.
    #
    # 2.6 passed the filtering configuration positionally through
    # connection_resolver:
    #
    #     (resolver, connection, default_manager, max_limit,
    #      enforce_first_or_last, filterset_class, filtering_args, root, info)
    #
    # 2.16 moved filtering into get_queryset_resolver() and passes a single
    # `queryset_resolver` instead:
    #
    #     (resolver, connection, default_manager, queryset_resolver,
    #      max_limit, enforce_first_or_last, root, info)
    #
    # Keeping the old signature did not fail at import or change the schema
    # — it failed at *query* time, with "connection_resolver() missing 1
    # required positional argument: 'info'", which would have left every
    # paginated list in the reader empty. See DJANGO-3.2-PLAN.md §4.1.
    #
    # The behaviour this class exists for is unchanged: when the caller
    # supplies neither `first` nor `last`, default `first` to `max_limit`.
    @classmethod
        ...
        meta_options.update(
            {
                ...
                # LOCAL CHANGE (2026-08-14): graphene-django 3 requires
                # every DjangoObjectType.Meta to declare fields or
                # exclude. "__all__" reproduces the implicit behaviour
                # 2.x had, which is what keeps the schema — and so the
                # frontend contract — identical. A hand-curated subset
                # would risk silently dropping a field.
                "fields": "__all__",
            }
        )
