...
# LOCAL CHANGE (2026-08-14): was django_jsonfield_backport. Django has
# shipped models.JSONField since 3.1, and graphene-django converts it to
# the same JSONString scalar the backport shim produced, so the GraphQL
# wire format is unchanged. See DJANGO-3.2-PLAN.md §4.4.
...
class TextAnnotation(models.Model):
    kind = models.CharField(
        # LOCAL CHANGE (2026-08-14): was max_length=7, which cannot hold
        # TEXT_ANNOTATION_KIND_SYNTAX_TREE ("syntax-tree", 11 characters).
        # Django 3.2 added fields.E009 and refuses to start on it. SQLite
        # does not enforce varchar length, so it went unnoticed — on
        # Postgres, storing a syntax-tree annotation would have failed
        # outright. No rows are affected here: the table is empty.
        max_length=11,
